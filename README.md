...
| `CC_API_KEYS` | *(unset)* | Ordered account pool, comma/space separated → `apiKeys` |
| `CC_KEY_FAILOVER` | `true` | `false` disables cross-account failover → `keyFailover` |
| `CC_KEY_COOLDOWN_MS` | `604800000` | Max cooldown, also the fixed quota/auth cooldown → `keyCooldownMs` |
| `CC_KEY_COOLDOWN_LADDER_MS` | `300000,3600000,43200000,86400000,604800000` | Escalating cooldown rungs (ms) → `keyCooldownLadderMs` |
...
| **Environment** | `x-cli-environment: production`, `x-taste-learning: "false"`, `User-Agent: cli` (no `x-co-flag` — aligned with CLI 1.53.1) |
