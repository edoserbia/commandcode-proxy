...
| `CC_API_KEYS` | *(未设置)* | 有序账号池，逗号/空白分隔 → `apiKeys` |
| `CC_KEY_FAILOVER` | `true` | 设为 `false` 关闭跨账号故障转移 → `keyFailover` |
| `CC_KEY_COOLDOWN_MS` | `604800000` | 冷却上限，也是额度/鉴权的固定冷却 → `keyCooldownMs` |
| `CC_KEY_COOLDOWN_LADDER_MS` | `300000,3600000,43200000,86400000,604800000` | 阶梯冷却各级（毫秒）→ `keyCooldownLadderMs` |
...
| **环境标识** | `x-cli-environment: production`、`x-taste-learning: "false"`、`User-Agent: cli`（无 `x-co-flag`，与 CLI 1.53.1 对齐）|
