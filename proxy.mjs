...
async function handleChatCompletions(req, res) {
  ...
  try {
    ...
    if (!acquired.response) {
      ...
      // 上游错误已由 forwardToCCWithFailover 完成归类/熔断/换账号；这里补一条带 code 的日志便于排查。
      log('error', 'CC API error', { status: mapped.status, code: mapped.code, body: (mapped.body?.error?.message || '').slice(0, 300) });
...
async function handleMessages(req, res) {
  ...
  try {
    ...
    if (!acquired.response) {
      const mapped = acquired.error || { status: 502, body: { error: { message: 'Upstream request failed', type: 'upstream_error' } } };
      // 上游错误已由 forwardToCCWithFailover 完成归类/熔断/换账号；这里补一条带 code 的日志便于排查。
      log('error', 'CC API error (Anthropic)', { status: mapped.status, code: mapped.code, body: (mapped.body?.error?.message || '').slice(0, 300) });
...
async function fetchModels(apiKey) {
  ...
  let response;
  try {
    response = await upstreamFetch(`${CFG.apiBase}/provider/v1/models`, {
      headers: {
        'Authorization': `Bearer ${apiKey}`,
        'x-cli-environment': 'production',
        'x-command-code-version': CC_VERSION,
